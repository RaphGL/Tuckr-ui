...
impl Page {
	...
	pub fn try_into_cli(self, groups: Vec<String>) -> Option<Cli> {
		match self {
			...
			Page::Add(exclude, force, adopt) => Some(Cli::Add {
				groups,
				exclude: exclude.unwrap_or_default(),
				force,
				adopt,
			}),
			Page::Rm(exclude) => Some(Cli::Rm {
				groups,
				exclude: exclude.unwrap_or_default(),
			}),
			Page::Set(exclude, force, adopt) => Some(Cli::Set {
				groups,
				exclude: exclude.unwrap_or_default(),
				force,
				adopt,
			}),
			Page::Push(f) => Some(Cli::Push {
				...
				ui.vertical_centered(|ui| {
					...
					if ui.button("Exacute").clicked() {
						let _ = match self
							.page
							.clone()
							.try_into_cli(self.groups.clone().unwrap_or(vec![r"\*".into()]))
						{
							Some(cli) => run(cli),
